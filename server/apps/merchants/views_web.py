...
class DashboardItemsView(MerchantRequiredMixin, StoreContextMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            StoreItem.objects.filter(store=active_store)
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(
            {
                "categories": ItemCategory.objects.filter(store=active_store).order_by(
                    "display_order", "name"
                ),
                "total_stores": context["stores"].count(),
                "has_next": (context["current_index"] + 1) < context["stores"].count(),
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "submit_category" in request.POST:
            ...
            if category_form.is_valid():
                ...
                return self.render_to_response(
                    self.get_context_data(category_form=category_form)
                )
        ...
        # User submitted the Manage Categories Form (bulk update/delete)
            ...
            for category in store_categories:
                ...
                if (
                    new_order
                    and new_order.isdigit()
                    and int(new_order) != category.display_order
                ):
            ...
            if categories_to_update:
                ItemCategory.objects.bulk_update(
                    categories_to_update, ["name", "display_order"]
                )
        ...
        # User submitted the Item Form
            ...
            if item_form.is_valid():
                ...
                return self.render_to_response(
                    self.get_context_data(item_form=item_form)
                )
...
class DashboardItemUpdateView(MerchantRequiredMixin, StoreContextMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse(
            "merchants:dashboard_items",
            kwargs={"store_index": self.kwargs.get("store_index", 0)},
        )
...
class DashboardPromotionListView(MerchantRequiredMixin, StoreContextMixin, ListView):
    ...
    def get_queryset(self):
        return Promotion.objects.filter(store=self.get_active_store()).order_by(
            "-created_at"
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["edit_form"] = PromotionForm(
            store=self.get_active_store(), auto_id="edit_%s"
        )
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "submit_promo" in request.POST:
            ...
            if create_form.is_valid():
                ...
                return redirect(
                    "merchants:dashboard_promotions", store_index=store_index
                )
            else:
                return self.render_to_response(
                    self.get_context_data(create_form=create_form)
                )
...
class DashboardPromotionUpdateView(
    MerchantRequiredMixin, StoreContextMixin, UpdateView
):
    model = Promotion
    ...
    def get_success_url(self):
        return reverse(
            "merchants:dashboard_promotions",
            kwargs={"store_index": self.kwargs.get("store_index", 0)},
        )


class DashboardLocationsAndHoursView(
    MerchantRequiredMixin, StoreContextMixin, TemplateView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        existing_hours = {
            h.day_of_week: h for h in StoreOperatingHour.objects.filter(store=store)
        }
            ...
            days_info.append(
                {
                    ...
                    "open_time": (
                        hour_obj.open_time.strftime("%H:%M") if hour_obj else ""
                    ),
                    "close_time": (
                        hour_obj.close_time.strftime("%H:%M") if hour_obj else ""
                    ),
                }
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "submit_store_info" in request.POST:
            ...
            if form.is_valid():
                form.save()
                messages.success(
                    request, "Store location and details updated successfully!"
                )
                return redirect(
                    "merchants:dashboard_locations_and_hours",
                    store_index=kwargs.get("store_index"),
                )

            else:
                messages.error(
                    request,
                    "There was an error updating your store. Please check below.",
                )
            ...
            try:
                for day_name, day_int in days_mapping.items():
                    ...
                    if is_active:
                        ...
                        StoreOperatingHour.objects.filter(
                            store=store, day_of_week=day_int
                        ).delete()
                ...
                messages.error(
                    request, f"Error saving hours. Please ensure times are valid."
                )
...
class DashboardStaffView(MerchantRequiredMixin, StoreContextMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user != store.owner:
            ...
            return redirect(
                "merchants:dashboard_home", store_index=kwargs.get("store_index", 0)
            )
        return super().get(request, *args, **kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "invite_staff" and is_owner:
            ...
            if new_owner in store.staff.all():
                ...
                store.save()
                messages.success(
                    request, f"Ownership transferred to {new_owner.email}."
                )
        ...
        return redirect(
            "merchants:dashboard_staff", store_index=self.kwargs["store_index"]
        )
...
class AcceptInviteView(View):
    def get(self, request, token, *args, **kwargs):
        ...
        if not user:
            ...
            messages.success(
                request,
                f"You have been added to {invite.store.business_name} as staff.",
            )
...
class DashboardSettingsView(MerchantRequiredMixin, StoreContextMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["user_form"] = UserInfoForm(
            instance=getattr(user, "user_profile"), store=store
        )
...
class OnboardingStoreDetailView(
    MerchantRequiredMixin, MerchantHasStoresMixin, DetailView
):
    ...
    def post(self, request, store_id, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse(
                {
                    "status": "success",
                    "redirect_url": reverse("merchants:dashboard_redirect"),
                }
            )
        except (ValueError, json.JSONDecodeError):
            return JsonResponse(
                {"error": "Invalid request data processing layer"}, status=400
            )
